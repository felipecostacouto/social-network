...
class GraphHandler:
    ...
    def add_user(self, new_user: TwitterUser):
        self.DiGraph.add_weighted_edges_from(map(lambda user: (
            new_user.username, user.username, new_user.get_user_prestige()), new_user.get_users_following()))
        self.save_graph()
        self.pagerank = dict(sorted(nx.pagerank(self.DiGraph).items(), key=lambda item: item[1]))
